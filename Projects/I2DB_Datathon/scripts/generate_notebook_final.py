"""
Generate datathon_notebook_final.docx — complete technical reference document.
Updated to include competition test set analysis, retrain-on-100% workflow,
threshold decision, and final submission details.
"""
...
def add_image(path, width=5.5):
    if os.path.exists(path):
        ...
        doc.add_paragraph('[Image not found: {}]'.format(path))
...
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run('Final Technical Notebook')
run.font.size = Pt(16)
run.italic = True
...
toc_items = [
    ...
    '   3.3 Step 8: Train-Test Split and Internal Validation Strategy',
    '4. Modeling (Steps 9-12)',
    '   4.1 Step 9: Baseline Logistic Regression',
    '   4.2 Step 10: Advanced Models',
    '   4.3 Step 11: Hyperparameter Tuning',
    '   4.4 Step 12: Internal Validation Evaluation',
    '5. Interpretation (Steps 13-14)',
    '   5.1 Step 13: Feature Importance and SHAP',
    '   5.2 Step 14: Subgroup Fairness Analysis',
    '6. Competition Test Set and Final Submission',
    '   6.1 Receiving the Test Set',
    '   6.2 Retraining on 100% of Training Data',
    '   6.3 Test Set Sanity Checks',
...
add_table(
    ...
    [
        ...
        ['Evaluation', 'Round 1: Model performance (True/False accuracy). Round 2: Presentation to judges.'],
        ['Submission format', 'CSV file matching DM_Control_2025.csv format (patient ID + True/False)'],
    ]
)

doc.add_heading('Our Approach in Plain English', level=2)
doc.add_paragraph(
    'We used 12 months of electronic health record (EHR) data for 62,425 diabetes patients (the training set) '
    'to build a model that predicts which patients will have uncontrolled A1c in the following year. '
    'Our approach followed a systematic 16-step pipeline: explore the data, clean it, engineer clinically '
    'meaningful features, train multiple machine learning models, tune the best one, evaluate it fairly '
    'across demographic subgroups, and finally apply it to the unseen competition test set (15,607 patients).'
)
doc.add_paragraph(
    'The final model -- a tuned XGBoost classifier retrained on 100% of the training data -- achieved '
    'an AUC of 0.854 on internal validation and catches approximately 78% of patients who will become '
    'uncontrolled. We selected a threshold of 0.50, prioritizing sensitivity because this is a screening '
    'tool where missing an at-risk patient has greater clinical consequences than an unnecessary follow-up.'
)

doc.add_heading('Dataset Overview', level=2)
add_table(
    ['Property', 'Training Set', 'Competition Test Set'],
    [
        ['Patients', '62,425', '15,607'],
        ['Feature columns', '41', '41 (identical)'],
        ['Target variable', 'a1c 2025 Uncontrolled (True=10.6%, False=89.4%)', 'Unknown (to be predicted)'],
        ['Feature file', 'DM Features.csv', 'TEST SET DM Features.csv'],
        ['Target file', 'DM_Control_2025.csv', 'N/A'],
        ['Patient overlap', '--', '0 (completely separate patients)'],
    ]
)
...
doc.add_paragraph(
    'All exploration was performed on the training set (62,425 patients). The competition test set was '
    'not available during this phase. These findings informed every subsequent decision in the pipeline.'
)
...
doc.add_paragraph(
    ...
    'feature engineering. Every column is now numeric (int64 or float64). The same cleaning pipeline '
    'was later applied identically to the competition test set.'
)
...
add_table(
    ['Feature', 'Calculation', 'Clinical reasoning'],
    [
        ['bmi', 'Weight (kg) / (height (m))^2', 'Body mass index. Mean 32.6 (obese range). Only available for 23.5% of patients due to missingness in weight/height.'],
...
# --- Step 8 ---
doc.add_heading('3.3 Step 8: Train-Test Split and Internal Validation Strategy', level=2)

doc.add_paragraph(
    'To evaluate our model honestly before receiving the competition test set, we split the 62,425 '
    'training patients into an internal training set (80%) and an internal validation set (20%) using '
...
add_table(
    ...
    [
        ['Internal Train (80%)', '49,940', '44,634', '5,306', '10.62%'],
        ['Internal Validation (20%)', '12,485', '11,159', '1,326', '10.62%'],
    ]
)
...
bold_para('Important: This split was for internal validation only.')
doc.add_paragraph(
    'The 80/20 split allowed us to develop and evaluate our model with confidence. All model '
    'selection (Steps 9-10), hyperparameter tuning (Step 11), and performance evaluation (Step 12) '
    'were done using this split. Once we were satisfied with the model\'s performance, we retrained '
    'on the full 100% of training data before applying it to the competition test set (see Section 6). '
    'This is standard practice -- you validate on a subset, then train on everything for the final model.'
)
...
doc.add_paragraph(
    'All modeling was performed using the 80/20 internal split. The internal validation set (20%) '
    'was treated as a sealed envelope -- not used for any training or tuning decisions.'
)
...
doc.add_paragraph(
    ...
    'pay 8x more attention to misclassifying an uncontrolled patient than a controlled one. Without '
...
doc.add_paragraph(
    'We used RandomizedSearchCV with 80 parameter combinations x 5-fold cross-validation = 400 '
...
add_table(
    ...
    [
        ...
        ['scale_pos_weight', '8.41', 'Ratio of controlled to uncontrolled patients (44,634 / 5,306). Tells the model that missing an uncontrolled patient is 8.4x worse than a false alarm.'],
    ]
)
...
# --- Step 12 ---
doc.add_heading('4.4 Step 12: Internal Validation Evaluation', level=2)

doc.add_paragraph(
    'This is the moment of truth -- evaluating our final tuned model on the internal validation set of '
...
doc.add_paragraph(
    ...
    'neighborhood deprivation (ADI) using the internal validation set (where we have true labels).'
)
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 6: COMPETITION TEST SET AND FINAL SUBMISSION
# ══════════════════════════════════════════════════════════════════════════════
doc.add_heading('6. Competition Test Set and Final Submission', level=1)

doc.add_paragraph(
    'This section documents what happened after we received the competition test set -- the unseen '
    'patients we need to make predictions for. This is the culmination of all prior work.'
)

# --- 6.1 ---
doc.add_heading('6.1 Receiving the Test Set', level=2)

doc.add_paragraph(
    'The competition organizers released the test set (TEST SET DM Features.csv) approximately one '
    'week before the submission deadline. This file contains 15,607 new patients -- completely separate '
    'from the 62,425 patients in our training data.'
)

bold_para('Initial verification:')
add_table(
    ['Check', 'Result'],
    [
        ['Test set shape', '15,607 patients x 41 columns'],
        ['Columns match training?', 'Yes -- all 41 columns identical'],
        ['Patient ID overlap?', '0 -- completely separate patients'],
        ['Overall split', '~80/20 (62,425 train / 15,607 test)'],
    ]
)

doc.add_paragraph(
    'The fact that the column structure is identical means our cleaning and feature engineering '
    'pipeline can be applied without modification.'
)

# --- 6.2 ---
doc.add_heading('6.2 Retraining on 100% of Training Data', level=2)

doc.add_paragraph(
    'During model development (Steps 9-12), we used an 80/20 internal split: 80% for training, '
    '20% for validation. This was necessary to evaluate our model honestly. Now that we have the '
    'competition test set and our model architecture is finalized, we retrained on all 62,425 '
    'training patients.'
)

bold_para('Why retrain on 100%?')
doc.add_paragraph(
    'The 80/20 split was a development tool -- it let us test our model without peeking at the '
    'competition test set. Now that we know our approach works (AUC = 0.854 on internal validation), '
    'there is no reason to withhold 20% of the data. More training data means the model learns '
    'more patterns and generalizes better. This is standard practice in competition settings and '
    'was explicitly recommended by Adam Wilcox during office hours.'
)

bold_para('What stayed the same:')
doc.add_paragraph(
    'Everything except the amount of training data. Same cleaning pipeline, same 20 engineered '
    'features, same XGBoost hyperparameters (800 trees, learning_rate=0.01, max_depth=5, etc.), '
    'same imputation strategy (median). The only change is that the model now sees all 62,425 '
    'patients instead of 49,940.'
)

add_table(
    ['Property', '80% Model (development)', '100% Model (final)'],
    [
        ['Training patients', '49,940', '62,425'],
        ['Hyperparameters', 'Tuned via RandomizedSearchCV', 'Same (carried over)'],
        ['Features', '56 columns', '56 columns (identical)'],
        ['Internal AUC', '0.852 (on 20% holdout)', '0.854 (on same holdout, for reference)'],
    ]
)

# --- 6.3 ---
doc.add_heading('6.3 Test Set Sanity Checks', level=2)

doc.add_paragraph(
    'Before trusting our predictions, we verified that the competition test set looks similar to the '
    'training data. If the test patients were fundamentally different (different age distribution, '
    'different A1c ranges, different missingness patterns), our model might not generalize well. '
    'We performed three checks.'
)

bold_para('Check 1: Missingness patterns')
doc.add_paragraph(
    'We compared the percentage of missing values for every column between the training set and the '
    'competition test set. The maximum difference was 0.7 percentage points (A1c 2). This confirms '
    'the two datasets were drawn from the same population and processed identically.'
)

add_image('plots/final_missingness_comparison.png', 5.5)
doc.add_paragraph(
    'Figure 21: Missingness comparison between training (blue) and competition test (orange). '
    'The bars are nearly identical for every column -- no structural differences.'
)

bold_para('Check 2: Feature distributions')
doc.add_paragraph(
    'We compared the distributions of key features (A1c, age, medications, utilization) between '
    'training and test sets. All features were within 1-5% of each other.'
)

add_table(
    ['Feature', 'Training Mean', 'Test Mean', 'Difference'],
    [
        ['Age', '65.5', '65.6', '+0.1%'],
        ['A1c (latest)', '7.13', '7.10', '-0.5%'],
        ['A1c (mean)', '7.09', '7.06', '-0.5%'],
        ['N A1c tests', '1.60', '1.59', '-0.7%'],
        ['Total med classes', '0.83', '0.82', '-1.2%'],
        ['A1c above 9', '12.0%', '11.5%', '-4.3%'],
        ['On insulin', '33.6%', '34.0%', '+1.1%'],
        ['No medication', '42.3%', '42.1%', '-0.4%'],
        ['Any admission', '32.2%', '32.4%', '+0.7%'],
    ]
)

add_image('plots/final_feature_distributions.png', 5.5)
doc.add_paragraph(
    'Figure 22: Key feature distributions overlaid. Training (blue) and test (orange) overlap '
    'almost perfectly -- the patient populations are drawn from the same source.'
)

bold_para('Check 3: Prediction distribution comparison')
doc.add_paragraph(
    'We compared the model\'s predicted probability distributions on the internal validation set '
    '(where we know the answers) versus the competition test set (where we do not). The distributions '
    'have similar overall shape, confirming the model is behaving consistently.'
)

add_image('plots/final_test_vs_val_comparison.png', 5.5)
doc.add_paragraph(
    'Figure 23: Four-panel comparison of model predictions on internal validation vs competition test set. '
    'Top-left: probability distribution overlap. Top-right: cumulative distributions. '
    'Bottom-left: risk bucket breakdown. Bottom-right: feature mean ratios (all near 1.0).'
)

doc.add_paragraph(
    'The competition test set shows a more bimodal distribution (more patients at the extremes -- '
    'very low risk and high risk) compared to the internal validation set. This suggests the model '
    'is more "decisive" on the test set, which is not unusual when the model has been trained on '
    'the full dataset and thus has more confident learned patterns.'
)

# --- 6.4 ---
doc.add_heading('6.4 Threshold Selection', level=2)

doc.add_paragraph(
    'The model outputs a probability (0 to 1) for each patient. To convert this into a True/False '
    'prediction, we must choose a cutoff threshold. Any patient above the threshold is predicted as '
    '"Uncontrolled." This decision has significant clinical implications.'
)

bold_para('Why not just use 0.5?')
doc.add_paragraph(
    'We trained the model with scale_pos_weight=8.41, which tells the model to heavily penalize '
    'missing an uncontrolled patient. This is clinically appropriate but pushes the model\'s '
    'probability outputs upward. As a result, different thresholds produce very different tradeoffs:'
)

add_table(
    ['Threshold', 'Accuracy', 'F1 Score', 'Sensitivity', 'Precision', 'Predicted Rate'],
    [
        ['0.30', '52.6%', '0.300', '95.6%', '17.8%', '57.1%'],
        ['0.40', '67.9%', '0.372', '89.4%', '23.5%', '40.5%'],
        ['0.50 (selected)', '77.3%', '0.423', '78.4%', '29.0%', '28.7%'],
        ['0.60 (best F1)', '83.5%', '0.453', '64.6%', '34.9%', '19.6%'],
        ['0.70', '88.2%', '0.394', '36.0%', '43.5%', '8.8%'],
        ['0.80', '89.5%', '0.072', '3.8%', '55.4%', '0.7%'],
    ]
)

add_image('plots/step3_threshold_analysis.png', 5.5)
doc.add_paragraph(
    'Figure 24: Threshold analysis. Left: F1 and accuracy vs threshold. Center: precision-recall tradeoff. '
    'Right: predicted uncontrolled rate vs threshold (red dashed line = true 10.6% base rate).'
)

bold_para('Our decision: Threshold = 0.50')
doc.add_paragraph(
    'We selected 0.50 as our threshold for the following clinical reasons:'
)
doc.add_paragraph(
    '1. Sensitivity priority: This is a screening tool. In diabetes care, the cost of missing an '
    'at-risk patient -- progressive end-organ damage (retinopathy, nephropathy, neuropathy) over '
    'a full year without intervention -- far outweighs the cost of an additional follow-up for a '
    'patient who turns out to be well-controlled.'
)
doc.add_paragraph(
    '2. Catches 78% of at-risk patients: At this threshold, the model identifies approximately '
    '4 out of 5 patients who will become uncontrolled. The 22% it misses tend to be borderline '
    'cases (predicted probability 0.3-0.5) where the clinical signal is genuinely ambiguous.'
)
doc.add_paragraph(
    '3. Actionable false positives: Patients flagged as "uncontrolled" who turn out to be controlled '
    'would receive, at most, an earlier follow-up appointment or a phone call from a care coordinator. '
    'This is a low-cost, low-harm intervention compared to the consequences of missing a truly '
    'at-risk patient.'
)
doc.add_paragraph(
    '4. Standard default: 0.50 is the standard classification threshold and does not require '
    'additional justification for the competition evaluation.'
)

# --- 6.5 ---
doc.add_heading('6.5 Final Submission', level=2)

doc.add_paragraph(
    'We applied the complete pipeline to the competition test set:'
)

doc.add_paragraph('1. Loaded the 15,607 test patients from TEST SET DM Features.csv')
doc.add_paragraph('2. Applied the same cleaning pipeline (drop leakage column, encode categoricals, fix ADI)')
doc.add_paragraph('3. Engineered the same 20 features (A1c summaries, medication patterns, BMI, utilization)')
doc.add_paragraph('4. Aligned columns to match training (added any missing dummy columns, removed extras)')
doc.add_paragraph('5. Imputed missing values using the same median imputer fitted on training data')
doc.add_paragraph('6. Generated predicted probabilities using the 100%-trained XGBoost model')
doc.add_paragraph('7. Applied threshold of 0.50 to produce True/False predictions')

bold_para('Submission summary:')
add_table(
    ['Property', 'Value'],
    [
        ['File', 'final_submission.csv'],
        ['Format', 'Patient ID (index) + "a1c 2025 Uncontrolled" (True/False)'],
        ['Total patients', '15,607'],
        ['Predicted Uncontrolled (True)', '4,841 (31.0%)'],
        ['Predicted Controlled (False)', '10,766 (69.0%)'],
        ['Model', 'XGBoost trained on 100% of training data (62,425 patients)'],
        ['Threshold', '0.50'],
        ['Internal validation AUC', '0.854'],
    ]
)

bold_para('Why is the predicted rate (31%) higher than the training base rate (10.6%)?')
doc.add_paragraph(
    'This is expected and by design. We used scale_pos_weight=8.41, which tells the model to '
    'prioritize catching uncontrolled patients. At the 0.50 threshold, this produces more '
    '"True" predictions than the actual base rate. The model is being deliberately cautious -- '
    'it would rather flag a patient who turns out to be fine than miss a patient who is at risk. '
    'This is the same tradeoff that makes screening tests (mammography, PSA, rapid strep) have '
    'higher positive rates than the actual disease prevalence in the population.'
)

bold_para('Subgroup prediction check on competition test set:')
doc.add_paragraph(
    'Even without true labels, we verified the model\'s predictions are consistent across subgroups. '
    'The relative ordering matches what we observed in the training data: males have higher predicted '
    'uncontrolled rates than females, younger patients higher than older, consistent with known '
    'diabetes epidemiology.'
)

add_image('plots/final_subgroup_predictions.png', 5.5)
doc.add_paragraph(
    'Figure 25: Predicted uncontrolled rate by subgroup on the competition test set (orange) '
    'compared to actual rates in the training data (blue). The relative ordering is preserved: '
    'males > females, younger > older. The predicted rates are higher overall due to the '
    'sensitivity-prioritized threshold.'
)
...
add_table(
    ...
    [
        ...
        ['Step 9', 'Use class_weight="balanced" for logistic regression', 'Without class weighting, the model predicts "controlled" for everyone (89.4% accuracy, 0% recall). Balanced weighting tells the model that missing an uncontrolled patient is 8x worse than a false alarm.'],
        ['Step 10', 'Choose XGBoost for tuning over Random Forest', 'XGBoost typically outperforms Random Forest once tuned. It supports native NaN handling and has more tunable hyperparameters.'],
        ['Step 11', 'Use scale_pos_weight = 8.41', 'This is the ratio of controlled to uncontrolled patients. It penalizes false negatives proportionally to the imbalance.'],
        ['Step 11', 'learning_rate = 0.01 with 800 trees', 'Slower learning rate + more trees = better generalization. Each tree makes a small correction, reducing overfitting.'],
        ['Step 11', 'subsample = 0.7, colsample_bytree = 0.5', 'Randomness in training prevents overfitting. Each tree sees 70% of patients and 50% of features, forcing robust pattern learning.'],
        ['Final', 'Retrain on 100% of training data', 'The 80/20 split was for internal validation only. Once the model architecture was finalized, we retrained on all 62,425 patients to maximize learning before predicting on the competition test set. Recommended by competition organizers.'],
        ['Final', 'Threshold = 0.50 (sensitivity-prioritized)', 'This is a screening tool. Missing an at-risk patient (false negative) has far greater clinical consequences than an unnecessary follow-up (false positive). 0.50 catches ~78% of at-risk patients.'],
        ['Final', 'No re-exploration of test set', 'We do not have test set labels, so we cannot compute AUC, confusion matrices, or SHAP on the test set. All evaluation was done on the internal validation set. Sanity checks (feature distributions, missingness, prediction consistency) confirmed the test set is comparable.'],
    ]
)
...
glossary = [
    ...
    ['Sensitivity (Recall)',
     ...
     'Our model\'s sensitivity of ~78% means it catches roughly 4 out of 5 at-risk patients. '
    ...
    ['F1 Score',
     ...
     'Ranges from 0 to 1. We did NOT optimize for F1 because it treats false negatives and '
     'false positives as equally bad, which is not clinically appropriate for screening.'],
    ...
    ['Train-Test Split',
     ...
     'evaluate it on unseen data). We used two levels of splitting: (1) an internal 80/20 split '
     'for model development, and (2) the competition\'s own train/test split for final evaluation.'],
    ...
    ['scale_pos_weight',
     ...
     'costs 8.41x more than incorrectly flagging a controlled patient. This matches the ~1:8 '
     'class imbalance ratio.'],

    ['Threshold',
     'The cutoff point for converting a probability into a yes/no prediction. If a patient\'s '
     'predicted probability exceeds the threshold, they are classified as "Uncontrolled." '
     'A lower threshold catches more at-risk patients but also flags more false positives. '
     'We chose 0.50 to prioritize sensitivity for this screening application.'],
]
...
# SAVE
# ══════════════════════════════════════════════════════════════════════════════
doc.save('docs/datathon_notebook_final.docx')
print('Saved: docs/datathon_notebook_final.docx')
